...
does not define a red of its own: § 5 fixes the status hues.
...
| Item | Stage | Severity |
|---|---|---|
| `plugins/stream-kinds.ts` holds the tab descriptors and the alias match as plain data so the kind → tab rule can be exercised without rendering. There is still no test file for it — the check is ad hoc | `tester` | 🟡 |
| Neither Geist nor JetBrains Mono is bundled or fetched, so the stacks fall through to system fonts. Self-host or accept the fallback — decide before claiming the constitution's typography | `tokens` | 🟡 |
| `--color-destructive-foreground` on the new red measures 4.52:1 — passing but with no margin. Latent: no feature code uses `variant="destructive"` yet | `tokens` | 🟡 |
| `--color-canvas-tinted`, `--color-accent-active`, `--color-interactive-active`, `--font-display` are declared and unused | `janitor` | 🟡 |
| No dark-theme toggle although the `.dark` palette exists | `components` | 🟡 |
| `site/public/data` is generated by `prepare-data.mjs` and gitignored; the stale copy there still carries `conformance` indexes that `data/` no longer produces. Re-run `npm run prepare-data` — nothing to fix in the repo | — | 🟡 |
| No screenshot baselines on the shell or the product components — the only thing that would catch the two defects above automatically | — | 🟡 |
| `src/molcrafts_ci/jsonutil.py` and `persist.py` are not `ruff format` clean. CI runs `ruff check` only, so this is not a gate; pre-existing and untouched by the UI work | `janitor` | 🟡 |
| Hover, focus, transitions and the trend charts' geometry have never been looked at — no browser automation was reachable, so they are covered by types, build and static scan only. The layout and flows were reviewed by the author running it, which is what caught the inspector never opening and the rows not being clickable | — | 🟡 |
